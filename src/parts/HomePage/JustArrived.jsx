...
import Carousel from "../../components/Breadcrumb/Carousel";
  ...
  return (
    <section className="flex flex-col py-16">
      ...
      <div className="overflow-x-hidden px-4" id="carousel">
        ...
        {isLoading ? (
          <div
            className="card flex mx-4 flex-row relative"
            style={{
              paddingLeft:
          ...
          <Carousel refContainer={refContainer}>
            {data.data.map((item) => {
              return (
                <div key={item.id} className="px-4 relative card grou">
                  <div
                    className="rounded-xl overflow-hidden card-shadow relative"
                    style={{ width: 287, height: 386 }}
                  >
                    <div className="absolute opacity-0 group-hover:opacity-100 transition duration-200 flex items-center justify-center w-full h-full bg-slate-900 bg-opacity-30">
                  ...
                  <span className="">Rp {item.price.thousand()}</span>
                  <Link
